...
const MatchingMapLeaflet = ({ center, radiusKm, profiles, events, onProfilePress }) => {
  ...
  const hasValidCenter = Number.isFinite(Number(center?.latitude)) && Number.isFinite(Number(center?.longitude));
  ...
  if (!isClientReady || !hasValidCenter || !tileUrl) {
    return (
      <View style={{ minHeight: 440, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.06)', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <Text style={{ color: affairGoTheme.colors.text, fontSize: 18, fontWeight: '700', marginBottom: 8 }}>Karte wird geladen</Text>
        <Text style={{ color: affairGoTheme.colors.textMuted, textAlign: 'center', lineHeight: 22 }}>
          Die Kartenansicht wird initialisiert. Bitte versuche es in einem Moment erneut.
        </Text>
      </View>
    );
  }
